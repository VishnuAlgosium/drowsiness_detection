...
this model classifies the whole frame ("cigarette" vs "nocigarette")
...
class CigaretteDetector:
    ...
    def process(self, frame, now: float):
        ...
        this frame and update the confirm/cooldown alert state.

        Returns (cigarette_detected, confidence, alert_fired).
        """
        self.frame_num += 1
        should_run_inference = (self.frame_num % config.CIGARETTE_DETECT_EVERY_N_FRAMES) == 0

        if should_run_inference:
            results = self.model.predict(
                source=frame, imgsz=config.CIGARETTE_IMG_SIZE, verbose=False, device="cpu"
            )
